...
router = APIRouter(prefix="/api", tags=["TEST! Sender for test mass message"])
...
@router.get("/mass-mailing-telegram-test", response_model=SendMessageResponse, status_code=status.HTTP_200_OK)
...
async def send_mass_telegram_get(
    ...
    try:
        ...
        if not expat:
            chat_id_raw = get_rus_telegram_ids_test()
        else:
            chat_id_raw = get_expats_telegram_ids_test()
...
@router.get("/mass-mailing-max-test", response_model=SendMessageResponse, status_code=status.HTTP_202_ACCEPTED)
async def send_mass_mailing_max_get(
    ...
    try:
        ...
        if not expat:
            chat_id_raw = get_rus_max_ids_test()
        else:
            chat_id_raw = get_expats_max_ids_test()
